...
test('native cold launch remains branded until remote content or local recovery is visible', () => {
  ...
  assert.match(nativeDelegate, /scheduleReleaseWebViewConstraints\(\)/)
  assert.match(nativeDelegate, /installReleaseWebViewConstraintsIfReady\(\)/)
  assert.match(nativeDelegate, /releaseConstraintAttempts < 4/)
  assert.match(nativeDelegate, /view\.safeAreaInsets\.top > 0/)
  assert.match(nativeDelegate, /webView\.superview === view/)
  assert.match(nativeDelegate, /view\.layoutIfNeeded\(\)/)
  assert.match(nativeDelegate, /webView\.topAnchor\.constraint\(equalTo: view\.safeAreaLayoutGuide\.topAnchor\)/)
  assert.match(nativeDelegate, /NSLayoutConstraint\.deactivate\(inheritedConstraints\)/)
  assert.doesNotMatch(nativeDelegate, /override func viewDidLayoutSubviews\(\)/)
  assert.doesNotMatch(nativeDelegate, /override func viewDidAppear\(/)

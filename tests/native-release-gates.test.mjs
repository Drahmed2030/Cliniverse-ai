...
test('native cold launch remains branded until remote content or local recovery is visible', () => {
  ...
  assert.match(nativeDelegate, /override func webView\(with frame: CGRect, configuration: WKWebViewConfiguration\)/)
  assert.match(nativeDelegate, /WKUserScript/)
  assert.match(nativeDelegate, /injectionTime: \.atDocumentStart/)
  ...
  assert.match(nativeDelegate, /__cliniverseNativeSafeAreaContract/)
  assert.match(nativeDelegate, /MutationObserver/)
  assert.match(nativeDelegate, /getPropertyPriority\(name\)/)
  assert.match(nativeDelegate, /Number\.isFinite\(current\)/)
  assert.match(nativeDelegate, /state\.observer\.observe\(root/)
  assert.match(nativeDelegate, /data-release-header-inner/)
  assert.match(nativeDelegate, /headerPaddingTop/)
  assert.match(nativeDelegate, /\[CliniverseSafeArea\]/)
  assert.match(nativeDelegate, /CLINIVERSE_LAYOUT_DIAGNOSTICS/)
  assert.match(nativeDelegate, /cliniverse\.layout\.diagnostics/)
  assert.match(nativeDelegate, /releaseLayoutDiagnosticsScript/)
  assert.match(nativeDelegate, /releaseWebView\.isInspectable = true/)
  assert.match(nativeDelegate, /\[CliniverseLayoutDiagnostics\]/)
  assert.match(nativeDelegate, /adjustedContentInset/)
  assert.match(nativeDelegate, /visualViewport/)
  assert.match(nativeDelegate, /viewportMeta/)
  assert.match(nativeDelegate, /mediaQueries/)
  assert.match(nativeDelegate, /statusBarManager\?\.statusBarFrame\.maxY/)
  assert.match(nativeDelegate, /synchronizeReleaseSafeArea\(\)/)
  assert.match(nativeDelegate, /top: max\(measuredInsets\.top, bootstrapInsets\.top\)/)
  assert.match(nativeDelegate, /left: max\(measuredInsets\.left, bootstrapInsets\.left\)/)
  assert.match(nativeDelegate, /bottom: max\(measuredInsets\.bottom, bootstrapInsets\.bottom\)/)
  assert.match(nativeDelegate, /right: max\(measuredInsets\.right, bootstrapInsets\.right\)/)
  assert.doesNotMatch(nativeDelegate, /hasMeasuredInsets/)
  assert.match(nativeDelegate, /contentInsetAdjustmentBehavior = \.never/)
  assert.doesNotMatch(nativeDelegate, /NSLayoutConstraint\.deactivate/)
...
test('native shell uses HTTPS and blocks cleartext transport', () => {
  ...
  assert.equal(config.ios.contentInset, undefined)
  ...
  assert.match(offline, /--cliniverse-native-safe-area-top/)
})
...
test('native packaging is blocked until the canonical production origin matches the RC commit', () => {
  ...
  const packaging = workflow.slice(0, workflow.indexOf('  ios-screenshot-evidence:'))
  assert.match(workflow, /CM_COMMIT:\?Codemagic CM_COMMIT is required/)
  assert.match(packaging, /https:\/\/www\.cliniverseai\.com\/api\/release-contract/)
  assert.match(packaging, /verify-native-release-origin\.mjs/)
  assert.doesNotMatch(packaging, /allow-screenshot-native-delta/)
  assert.match(verifier, /payload\.commit === expectedCommit/)
  assert.match(verifier, /does not match RC commit/)

...
test('Apple route authenticates current account, uses configured verifier runtime, persists through trusted authority and never writes subscription directly', () => {
  const source = read('app/api/subscriptions/apple/verify/route.ts')
  assert.match(source, /authorization/)
  assert.match(source, /Bearer /)
  assert.match(source, /auth\.getUser\(token\)/)
  assert.match(source, /createConfiguredAppleVerifier/)
  ...
  assert.equal(source.includes('createUnavailableAppleVerifier'), false)
  assert.equal(source.includes('activatePro'), false)
  assert.equal(source.includes(".from('subscriptions')"), false)

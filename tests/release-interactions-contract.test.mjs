...
test('release surfaces respect iOS safe areas', () => {
  ...
  assert.match(releaseApp, /iPad\|iPhone\|iPod/)
  assert.match(releaseApp, /window\.innerWidth >= 768 \? 24 : 44/)
  assert.match(releaseApp, /useEffect\(\(\) =>/)
  assert.match(releaseApp, /syncNativeHeaderTopPadding\(\)/)
  assert.match(releaseApp, /window\.addEventListener\('resize', syncNativeHeaderTopPadding\)/)

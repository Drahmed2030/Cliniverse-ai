...
test('XCUITest captures the six approved surfaces and protects reviewer identity', () => {
  ...
  assert.match(source, /assertSystemChromeClear\("Today"\)/)
  assert.match(source, /assertSystemChromeClear\("PATIENT JOURNEY"\)/)
  assert.match(source, /app\.statusBars\.firstMatch/)
  assert.match(source, /observedStatusBarFrame\.maxY \+ 4/)
  assert.match(source, /statusBarFrame = \.zero/)
  assert.doesNotMatch(source, /statusBarFrame:\s*statusBar\.frame/)
  ...
  // Explore v2: the native capture waits on the Explore support line, which lives in the shared header.
  assert.match(source, /Reference, operations and advanced practice — when you need them\./)

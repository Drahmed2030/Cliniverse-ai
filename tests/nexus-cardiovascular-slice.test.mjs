...
test('Nexus cardiovascular slice is compiled but not exposed in the Apple release shell', async () => {
  ...
  for (const releaseSource of [releaseApp, releaseNav, ward]) {
    assert.doesNotMatch(releaseSource, /NexusCardiovascularSlice/)
  }
...
test('Nexus documentation records the separate release gate', async () => {
  const docs = await source('docs/NEXUS_CARDIOVASCULAR_SLICE_V1.md')

  assert.match(docs, /not imported by `ReleaseApp`/)
  assert.match(docs, /No API, database, model, or Supabase call/)
  assert.match(docs, /product, clinical-safety, privacy, accessibility, and Apple-release reviews/)
})

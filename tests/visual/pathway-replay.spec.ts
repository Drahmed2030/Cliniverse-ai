...
test('Clinical Studio completes the licensed real A4C lesson with reduced motion', async ({ page }, testInfo) => {
  ...
  const reviewPlan = page.getByRole('region', { name: 'Your review plan' })
  await expect(reviewPlan).toContainText('Focus: Learning scope.')
  await expect(reviewPlan.locator('time')).toHaveCount(3)
  const exportLink = reviewPlan.getByRole('link', { name: 'Download receipt and review plan' })
  const exportUrl = await exportLink.getAttribute('href')
  const exported = JSON.parse(decodeURIComponent(exportUrl!.split(',').slice(1).join(',')))
  expect(exported.reviewPlan.receiptId).toBe(exported.receipt.receiptId)
  expect(exported.reviewPlan.reviews.map((item: { afterDays: number }) => item.afterDays)).toEqual([1, 3, 7])
  const downloadEvent = page.waitForEvent('download')
  await exportLink.click()
  expect((await downloadEvent).suggestedFilename()).toBe(`cliniverse-${exported.receipt.receiptId}-review.json`)

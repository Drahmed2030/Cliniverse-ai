    ...
    <div style={{minHeight:'100vh',width:'100vw',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'radial-gradient(ellipse at 30% 20%, #1e3d52 0%, #162e3e 40%, #000510 100%)',fontFamily:'-apple-system,sans-serif',padding:'0 32px',textAlign:'center'}}>
      <div style={{animation:'fadeIn 0.8s ease',maxWidth:420,width:'100%'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12,maxWidth:360,margin:'0 auto'}}>
          <button onClick={()=>setScreen('signin')} style={{padding:'18px 32px',borderRadius:16,border:'none',cursor:'pointer',background:'linear-gradient(135deg,#00C4B4,#0a84ff)',color:'white',fontSize:17,fontWeight:700,boxShadow:'0 8px 40px rgba(0,196,180,0.5)'}}>Enter Hospital →</button>
    ...
    <div style={{minHeight:'100vh',width:'100vw',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'radial-gradient(ellipse at 70% 80%, #1e3d52 0%, #162e3e 40%, #000510 100%)',fontFamily:'-apple-system,sans-serif',padding:'40px 24px'}}>
      <div style={{animation:'fadeIn 0.6s ease',width:'100%',maxWidth:420}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input placeholder="doctor@hospital.com" style={{width:'100%',padding:'16px',borderRadius:14,border:'1px solid rgba(0,196,180,0.20)',background:'rgba(255,255,255,0.14)',color:'white',fontSize:15,outline:'none',boxSizing:'border-box'}}/>
        </div>
        <div style={{marginBottom:28}}>
          ...
          <input type="password" placeholder="••••••••" style={{width:'100%',padding:'16px',borderRadius:14,border:'1px solid rgba(0,196,180,0.20)',background:'rgba(255,255,255,0.14)',color:'white',fontSize:15,outline:'none',boxSizing:'border-box'}}/>
        </div>
    ...
    return(
      <div style={{minHeight:'100vh',width:'100vw',background:'radial-gradient(ellipse at 30% 20%, #1e3d52 0%, #162e3e 40%, #000510 100%)',fontFamily:'-apple-system,sans-serif',paddingBottom:40,position:'relative'}}>
        ...
        <div style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:0,overflow:'hidden'}}>
          <svg width="100%" height="100%" viewBox="0 0 400 800" fill="none" style={{opacity:0.05}}>
            <text x="140" y="380" fontSize="260" fontWeight="900" fill="#FFFFFF" fontFamily="-apple-system,sans-serif" letterSpacing="-8">C</text>
            <text x="290" y="380" fontSize="200" fontWeight="900" fill="#007AFF" fontFamily="-apple-system,sans-serif">AI</text>
            <rect x="18" y="18" width="44" height="44" rx="12" fill="none" stroke="#007AFF" strokeWidth="1.5"/>
            <rect x="338" y="18" width="34" height="34" rx="9" fill="none" stroke="#D4A847" strokeWidth="1.2"/>
            <rect x="18" y="730" width="38" height="38" rx="10" fill="none" stroke="#D4A847" strokeWidth="1.2"/>
            <rect x="346" y="730" width="30" height="30" rx="8" fill="none" stroke="#007AFF" strokeWidth="1"/>
            <text x="22" y="160" fontSize="45" fontWeight="900" fill="#FFFFFF" fontFamily="-apple-system,sans-serif" opacity="0.4">C</text>
            <text x="340" y="600" fontSize="45" fontWeight="900" fill="#007AFF" fontFamily="-apple-system,sans-serif" opacity="0.4">AI</text>
          </svg>
        </div>
      {/* ── C AI BACKGROUND PATTERN ── */}
      ...
      <div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none',zIndex:0,overflow:'hidden',opacity:0.045}}>
        ...
        <div style={{padding:'16px 20px',maxWidth:640,margin:'0 auto',position:'relative',zIndex:1}}>
          ...
          <button onClick={()=>{setShowAI(s=>!s);setAiHistory([]);setAiResponse('')}} style={{width:'100%',padding:'14px',borderRadius:18,border:'none',cursor:'pointer',background:'linear-gradient(135deg,#1e3d52,#162e3e)',color:'white',fontSize:14,fontWeight:700,marginBottom:10,boxShadow:'0 6px 24px rgba(0,196,180,0.4)',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
            ...
            <div style={{background:'linear-gradient(145deg,rgba(15,23,42,0.97),rgba(10,15,30,0.99))',borderRadius:20,padding:18,marginBottom:14,border:'1px solid rgba(0,196,180,0.3)',boxShadow:'0 8px 40px rgba(0,196,180,0.3)'}}>
              ...
              <div style={{display:'flex',gap:8,marginTop:8}}>
                <input
                  ...
                  style={{flex:1,padding:'12px 16px',borderRadius:14,border:'1px solid rgba(0,196,180,0.20)',background:'rgba(255,255,255,0.14)',color:'white',fontSize:13,outline:'none'}}
                />
                <button
                  onClick={()=>askAI(`${c.title}: ${c.presentation}. Vitals: BP ${c.vitals.bp}, HR ${c.vitals.hr}, O2 ${c.vitals.o2}%. ECG: ${c.ecg}`)}
                  disabled={aiLoading||!aiQuestion.trim()}
                  style={{width:46,height:46,borderRadius:14,border:'none',background:aiLoading||!aiQuestion.trim()?'rgba(0,196,180,0.20)':'linear-gradient(135deg,#00C4B4,#0a84ff)',color:'white',fontSize:18,cursor:aiLoading||!aiQuestion.trim()?'not-allowed':'pointer',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}}
                >→</button>
              </div>
            </div>
          )}
    ...
    <div style={{minHeight:'100vh',width:'100vw',background:'radial-gradient(ellipse at 30% 20%, #1e3d52 0%, #162e3e 40%, #000510 100%)',fontFamily:'-apple-system,BlinkMacSystemFont,SF Pro Display,sans-serif',display:'flex',flexDirection:'column',position:'relative'}}>
      ...
      <header style={{background:T.headerBg,backdropFilter:'blur(40px) saturate(180%)',WebkitBackdropFilter:'blur(40px) saturate(180%)',padding:'0 14px',height:48,display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:100,borderBottom:`0.5px solid ${T.headerBorder}`}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
          ...
          <div onClick={()=>setTab('profile')} style={{width:30,height:30,borderRadius:'50%',background:'linear-gradient(135deg,#00C4B4,#0a84ff)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontSize:13,cursor:'pointer',flexShrink:0,boxShadow:'0 2px 10px rgba(0,196,180,0.5)'}}>👤</div>
        </div>
      </header>
        ...
        <div onClick={()=>setCheckoutUrl('')} style={{position:'fixed',inset:0,zIndex:99999,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(12px)',display:'flex',flexDirection:'column',justifyContent:'flex-end'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#1e2d40',borderRadius:'28px 28px 0 0',border:'1px solid rgba(36,63,82,0.65)',height:'90vh',overflow:'hidden'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 20px',borderBottom:'1px solid rgba(36,63,82,0.65)'}}>
              ...
              <button onClick={()=>setCheckoutUrl('')} style={{background:'rgba(255,255,255,0.14)',border:'none',color:'white',width:32,height:32,borderRadius:'50%',cursor:'pointer',fontSize:18}}>×</button>
            </div>

          ...
          <div style={{paddingBottom:100}}>
            <div style={{padding:'16px 16px 0',marginBottom:16}}>
              <div style={{background:'linear-gradient(145deg,rgba(15,5,40,0.97),rgba(25,8,55,0.95))',borderRadius:24,padding:20,border:'1px solid rgba(139,92,246,0.2)'}}>
                <div style={{fontSize:11,color:'rgba(255,255,255,0.35)',letterSpacing:2,marginBottom:12,fontWeight:700}}>DAILY PROGRESS RINGS</div>
                <div style={{display:'flex',alignItems:'center',gap:20}}>
                  <div style={{position:'relative',width:110,height:110,flexShrink:0}}>
                    <svg width="110" height="110" viewBox="0 0 110 110" style={{transform:'rotate(-90deg)'}}>
                      <circle cx="55" cy="55" r="48" fill="none" stroke="rgba(10,132,255,0.15)" strokeWidth="8"/>
                      <circle cx="55" cy="55" r="48" fill="none" stroke="#0a84ff" strokeWidth="8" strokeLinecap="round" strokeDasharray={String(Math.min(casesCompleted*30,301))+' 302'} style={{filter:'drop-shadow(0 0 6px #0a84ff)'}}/>
                      <circle cx="55" cy="55" r="37" fill="none" stroke="rgba(48,209,88,0.15)" strokeWidth="8"/>
                      <circle cx="55" cy="55" r="37" fill="none" stroke="#30d158" strokeWidth="8" strokeLinecap="round" strokeDasharray={String(Math.min(mcqCorrect*23,232))+' 233'} style={{filter:'drop-shadow(0 0 6px #30d158)'}}/>
                      <circle cx="55" cy="55" r="26" fill="none" stroke="rgba(255,69,58,0.15)" strokeWidth="8"/>
                      <circle cx="55" cy="55" r="26" fill="none" stroke="#ff453a" strokeWidth="8" strokeLinecap="round" strokeDasharray={String(Math.min(xp*0.5,163))+' 164'} style={{filter:'drop-shadow(0 0 6px #ff453a)'}}/>
                    </svg>
            ...
            <div style={{padding:'0 16px',marginBottom:16}}>
              {(()=>{const h=new Date().getHours();const isMorning=h>=5&&h<12;const isEvening=h>=18||h<5;const accent=isMorning?'#ffd60a':isEvening?'#0a84ff':'#30d158';return(<div style={{background:'rgba(255,255,255,0.04)',borderRadius:20,padding:16,border:'1px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',gap:12}}><span style={{fontSize:28}}>{isMorning?'🌅':isEvening?'🌙':'☀️'}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:800,color:'white',marginBottom:2}}>{isMorning?'Morning Brief':isEvening?'Night Shift Mode':'Afternoon Challenge'}</div><div style={{fontSize:11,color:'rgba(255,255,255,0.


4)'}}>{isMorning?'MCQ review + quick cases':isEvening?'Complex emergency cases':'Rapid Fire is live'}</div></div><button onClick={()=>{setTab('tools');setToolTab(isMorning?'aigen':isEvening?'codeblue':'rapid')}} style={{background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:12,padding:'8px 14px',fontSize:12,fontWeight:700,color:accent,cursor:'pointer',flexShrink:0,fontFamily:'inherit'}}>Go</button></div>)})()}
            </div>
            <div style={{padding:'0 16px',marginBottom:16}}>
            ...
            <div style={{padding:'0 16px',marginBottom:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>
                {[{icon:'📋',label:'SBAR',action:()=>setTab('workshop'),color:'#0a84ff'},{icon:'⚡',label:'Rapid',action:()=>{setTab('tools');setToolTab('rapid')},color:'#ff453a'},{icon:'📈',label:'ECG',action:()=>{setTab('tools');setToolTab('ecg')},color:'#30d158'},{icon:'🧮',label:'Calc',action:()=>{setTab('tools');setToolTab('calc')},color:'#ff9f0a'}].map((q,i)=>(
                  <div key={i} onClick={q.action} style={{background:q.color+'12',border:'1px solid '+q.color+'25',borderRadius:16,padding:'12px 8px',textAlign:'center',cursor:'pointer'}}>
                  ...
                  {openAccordion===section.key&&(
                    <div style={{display:'flex',gap:10,overflowX:'auto',paddingBottom:6,scrollbarWidth:'none'}}>
                      {section.cases.map((c:any)=>(
                        <div key={c.id} onClick={()=>{if(!c.free&&!isPro){setShowUpgrade(true);return}setActiveCase(c.id)}} style={{flexShrink:0,width:160,background:'rgba(255,255,255,0.04)',borderRadius:18,padding:'14px',border:'1px solid '+c.color+'25',cursor:'pointer',opacity:!c.free&&!isPro?0.7:1}}>

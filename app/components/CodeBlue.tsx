    ...
    <div style={{ fontFamily: '-apple-system, sans-serif', paddingBottom: 20 }}>
      ...
      {phase==='intro'&&(
        <div style={{ background:'rgba(255,255,255,0.04)', backdropFilter:'blur(20px)', borderRadius:20, padding:20, border:'1px solid rgba(139,92,246,0.15)', boxShadow:'0 4px 24px rgba(0,0,0,0.07)' }}>
          ...
          <h3 style={{ fontSize:18, fontWeight:800, color:'white', textAlign:'center', marginBottom:6 }}>{scenario.title}</h3>
      ...
      {phase==='running'&&(
        <div>
          ...
          {/* Vitals Monitor */}
          <div style={{ background:'#0a1628', borderRadius:14, padding:12, marginBottom:10, border:'1px solid rgba(0,255,157,0.15)' }}>
      ...
      {phase==='success'&&(
        <div style={{ background:'rgba(48,209,88,0.1)', backdropFilter:'blur(20px)', borderRadius:20, padding:24, textAlign:'center', border:'2px solid rgba(22,163,74,0.3)', animation:'fadeIn 0.5s ease' }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            ...
            <button onClick={()=>{setScenarioIdx(i=>(i+1)%SCENARIOS.length);setPhase('intro')}} style={{ flex:2, padding:'14px', borderRadius:14, border:'none', background:'linear-gradient(135deg,#30d158,#0a84ff)', color:'white', fontSize:14, fontWeight:700, cursor:'pointer' }}>Next →</button>
          </div>
        </div>
      )}
      ...
      {phase==='failed'&&(
        <div style={{ background:'rgba(255,69,58,0.1)', backdropFilter:'blur(20px)', borderRadius:20, padding:24, textAlign:'center', border:'2px solid rgba(220,38,38,0.3)', animation:'fadeIn 0.5s ease' }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={startScenario} style={{ flex:2, padding:'14px', borderRadius:14, border:'none', background:'linear-gradient(135deg,#ff3b30,#ff6b35)', color:'white', fontSize:14, fontWeight:700, cursor:'pointer' }}>🔄 Try Again</button>

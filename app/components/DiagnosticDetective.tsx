...
export default function DiagnosticDetective({ onXP }: { onXP?: (n:number)=>void }) {
  ...
  if (phase === 'intro') return (
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      <div style={{background:'linear-gradient(135deg,rgba(191,90,242,0.12),rgba(10,132,255,0.08))',borderRadius:22,padding:'20px',marginBottom:16,border:'1px solid rgba(191,90,242,0.25)',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',top:-20,right:-20,width:100,height:100,borderRadius:'50%',background:'radial-gradient(circle,rgba(191,90,242,0.25),transparent 70%)',pointerEvents:'none'}}/>
        <div style={{fontSize:11,color:'rgba(191,90,242,0.8)',fontWeight:700,letterSpacing:1,textTransform:'uppercase',marginBottom:6}}>🕵️ NEW MODE</div>
        ...
        <div style={{display:'flex',gap:8}}>
          <div style={{flex:1,background:'rgba(255,255,255,0.11)',borderRadius:12,padding:'10px',border:'1px solid rgba(255,255,255,0.18)',textAlign:'center'}}>
      ...
      {CASES.map(c=>(
        <div key={c.id} onClick={()=>startCase(c)}
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            ...
            <span style={{fontSize:10,padding:'3px 10px',borderRadius:8,background:'rgba(139,92,246,0.25)',color:'#c4b5fd',fontWeight:700}}>+{c.xpReward} XP</span>
          </div>
        </div>
      ))}
    </div>
  ...
  if (phase === 'history') return (
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        <button onClick={()=>setPhase('intro')} style={{background:'rgba(139,92,246,0.25)',border:'1px solid rgba(139,92,246,0.3)',borderRadius:12,color:'#c4b5fd',padding:'8px 14px',fontSize:13,cursor:'pointer',fontWeight:600}}>← Back</button>
        <div style={{flex:1}}>
      ...
      <div style={{background:`${c.color}10`,borderRadius:20,padding:'18px',marginBottom:14,border:`1px solid ${c.color}30`,boxShadow:`0 6px 24px ${c.color}10`}}>
        ...
        <div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.85}}>{c.initialHistory}</div>
      </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            {ordered.map(inv=>(
              <div key={inv.id} style={{background:inv.critical?`${c.color}08`:C.card,borderRadius:14,padding:'12px 14px',marginBottom:8,border:`1px solid ${inv.critical?c.color+'30':C.border}`}}>
                ...
                <div style={{fontSize:12,color:'rgba(255,255,255,0.75)',lineHeight:1.6}}>{inv.result}</div>
              </div>
            ))}
          </div>
        )}
              ...
              {c.investigations.filter(i=>i.category===cat).map(inv=>{
                ...
                return (
                  <div key={inv.id} onClick={()=>!done&&canAfford&&orderTest(inv)}
                    style={{background:done?`${c.color}08`:C.card,borderRadius:14,padding:'12px 14px',border:done?`1px solid ${c.color}30`:canAfford?`1px solid ${C.border}`:'1px solid rgba(255,255,255,0.11)',cursor:done||!canAfford?'default':'pointer',opacity:done?0.7:canAfford?1:0.4,display:'flex',alignItems:'center',gap:12,transition:'all 0.2s'}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      {done&&<div style={{fontSize:11,color:'rgba(255,255,255,0.6)',marginTop:2,lineHeight:1.4}}>{inv.result}</div>}
                    </div>
  ...
  if (phase === 'diagnose') return (
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        <button onClick={()=>setPhase('investigate')} style={{background:'rgba(139,92,246,0.25)',border:'1px solid rgba(139,92,246,0.3)',borderRadius:12,color:'#c4b5fd',padding:'8px 14px',fontSize:13,cursor:'pointer',fontWeight:600}}>← More Tests</button>
        <div style={{flex:1}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
        {c.ddx.map((dx,i)=>(
          <div key={i} onClick={()=>setSelectedDx(i)}
            ...
            <div style={{width:30,height:30,borderRadius:9,background:selectedDx===i?`${c.color}25`:'rgba(255,255,255,0.15)',border:`1px solid ${selectedDx===i?c.color:'rgba(255,255,255,0.1)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,color:selectedDx===i?c.color:C.muted,flexShrink:0}}>{i+1}</div>
      ...
      <button onClick={submitDx} disabled={selectedDx===null}
        style={{width:'100%',padding:'16px',borderRadius:18,border:'none',background:selectedDx!==null?'linear-gradient(135deg,#8b5cf6,#0a84ff)':'rgba(255,255,255,0.15)',color:'white',fontSize:15,fontWeight:800,cursor:selectedDx!==null?'pointer':'not-allowed',boxShadow:selectedDx!==null?'0 6px 24px rgba(139,92,246,0.4)':'none',opacity:selectedDx!==null?1:0.5}}>
        ✅ Submit Diagnosis
      </button>
    </div>
  )
  ...
  if (phase === 'result') {
    ...
    return (
      <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
        <div style={{background:correct?'linear-gradient(145deg,rgba(48,209,88,0.15),rgba(10,132,255,0.08))':'linear-gradient(145deg,rgba(255,69,58,0.15),rgba(139,92,246,0.08))',borderRadius:24,padding:'28px 20px',marginBottom:16,border:correct?'1px solid rgba(48,209,88,0.25)':'1px solid rgba(255,69,58,0.25)',textAlign:'center',boxShadow:correct?'0 8px 32px rgba(48,209,88,0.15)':'0 8px 32px rgba(255,69,58,0.15)'}}>
          ...
          <div style={{display:'flex',gap:16,justifyContent:'center',marginBottom:8}}>
            ...
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:22,fontWeight:900,color:'#0a84ff'}}>{ordered.length}</div>
        ...
        <div style={{background:`${c.color}08`,borderRadius:18,padding:'16px',marginBottom:12,border:`1px solid ${c.color}20`}}>
          ...
          <div style={{fontSize:13,color:'rgba(255,255,255,0.8)',lineHeight:1.8}}>{c.finalAdvice}</div>
        </div>

        {/* Key learning */}
        <div style={{background:'rgba(10,132,255,0.06)',borderRadius:18,padding:'16px',marginBottom:16,border:'1px solid rgba(10,132,255,0.15)'}}>
          <div style={{fontSize:10,color:'#0a84ff',fontWeight:700,marginBottom:10,letterSpacing:0.5}}>💡 KEY LEARNING</div>
          {c.keyLearning.map((l,i)=>(
            <div key={i} style={{display:'flex',gap:8,marginBottom:8,alignItems:'flex-start'}}>
              <span style={{color:'#30d158',flexShrink:0,marginTop:2}}>✓</span>
              <span style={{fontSize:12,color:'rgba(255,255,255,0.75)',lineHeight:1.6}}>{l}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{display:'flex',gap:10}}>
          ...
          <button onClick={()=>setPhase('intro')} style={{flex:1,padding:'14px',borderRadius:16,border:'none',background:'linear-gradient(135deg,#bf5af2,#8b5cf6)',color:'white',fontSize:14,fontWeight:700,cursor:'pointer',boxShadow:'0 6px 20px rgba(191,90,242,0.4)'}}>🕵️ New Case</button>
        </div>
      </div>
    )
  }

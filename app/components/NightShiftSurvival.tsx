    ...
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      ...
      {showTip&&(
        <div style={{background:'rgba(255,214,10,0.08)',borderRadius:14,padding:'12px 14px',marginBottom:14,border:'1px solid rgba(255,214,10,0.2)'}}>
          ...
          <div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{sc.tips[caseIdx % sc.tips.length]}</div>
        </div>
      )}

      <button onClick={startCase}
        style={{width:'100%',padding:'16px',borderRadius:18,border:'none',background:`linear-gradient(135deg,${cc.color},${cc.color}bb)`,color:'var(--text-primary, #0A1628)',fontSize:15,fontWeight:800,cursor:'pointer',boxShadow:`0 6px 24px ${cc.color}44`,display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
      ...
      <div style={{display:'flex',gap:10,marginBottom:14}}>
        ...
        <div style={{flex:1,background:C.card,borderRadius:16,padding:'12px',border:`1px solid ${timerColor}30`,textAlign:'center'}}>
          <div style={{width:52,height:52,borderRadius:'50%',background:`conic-gradient(${timerColor} ${timerPct*3.6}deg, rgba(36,63,82,0.50) 0deg)`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 6px',boxShadow:`0 0 16px ${timerColor}44`}}>
            <div style={{width:42,height:42,borderRadius:'50%',background:'var(--bg-card,rgba(255,255,255,0.06))',display:'flex',alignItems:'center',justifyContent:'center'}}>
  ...
  if (phase === 'gameover') return (
    <div style={{fontFamily:'-apple-system,sans-serif',textAlign:'center',padding:'40px 20px'}}>
      ...
      <div style={{display:'flex',gap:10,justifyContent:'center'}}>
        ...
        <button onClick={()=>setPhase('menu')} style={{padding:'14px 24px',borderRadius:16,border:'none',background:'var(--bg-base,#F7F9FC)',color:'var(--text-primary, #0A1628)',fontSize:14,fontWeight:700,cursor:'pointer'}}>📋 Shifts</button>
      </div>
    </div>
  )
    ...
    return (
      <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
        ...
        <div style={{display:'flex',gap:10,marginTop:16}}>
          ...
          <button onClick={()=>setPhase('menu')} style={{flex:1,padding:'14px',borderRadius:16,border:'none',background:'var(--bg-base,#F7F9FC)',color:'var(--text-primary, #0A1628)',fontSize:14,fontWeight:700,cursor:'pointer',boxShadow:'0 6px 20px rgba(139,92,246,0.4)'}}>🌙 All Shifts</button>
        </div>
      </div>
    )
  }

    ...
    <div style={{padding:'0 16px',paddingBottom:100}}>
      ...
      <div style={{display:'flex',gap:6,overflowX:'auto',marginBottom:16,paddingBottom:4,scrollbarWidth:'none'}}>
        {CATS.map(c=>(
          <button key={c} onClick={()=>setCat(c)}
            style={{flexShrink:0,padding:'6px 14px',borderRadius:20,border:cat===c?'1px solid rgba(139,92,246,0.5)':'1px solid rgba(255,255,255,0.18)',background:cat===c?'rgba(139,92,246,0.3)':'rgba(255,255,255,0.12)',color:cat===c?'white':'rgba(255,255,255,0.5)',fontSize:12,fontWeight:cat===c?700:400,cursor:'pointer',fontFamily:'inherit'}}>
            {c}
          </button>
        ))}
      </div>
      {loading ? (
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {filtered.map(lab=>(
            <div key={lab.id} style={{background:'rgba(255,255,255,0.14)',borderRadius:16,padding:'12px 14px',border:'1px solid rgba(36,63,82,0.65)',display:'flex',alignItems:'center',gap:12}}>

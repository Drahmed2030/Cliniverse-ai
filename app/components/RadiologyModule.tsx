...
const CXR_FINDINGS = [
  ...
  { finding:'Pulmonary Oedema', criteria:'Bat-wing shadowing, Kerley B lines, pleural effusions', causes:'Left ventricular failure, ARDS, fluid overload', icon:'💧', color:'#00C4B4', nextStep:'Furosemide, O2, BNP, echo' },
      ...
      <div style={{background:'rgba(255,255,255,0.14)',borderRadius:18,padding:'14px 16px',border:`1px solid ${C.border}`}}>
        ...
        {['A — Airway: trachea midline?','B — Breathing: lung fields symmetric?','C — Cardiac: CTR, borders, shape','D — Diaphragm: right higher, no free air','E — Everything else: bones, soft tissue, lines'].map((s,i)=>(
          <div key={i} style={{display:'flex',gap:8,marginBottom:6}}>
            ...
            <span style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{s}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        <div key={i} onClick={()=>setExpandedCXR(expandedCXR===i?null:i)}
          ...
          {expandedCXR===i&&(
            <div style={{borderTop:'1px solid rgba(36,63,82,0.50)',paddingTop:10,display:'flex',flexDirection:'column',gap:8}}>
              <div style={{background:'rgba(255,255,255,0.14)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(36,63,82,0.65)'}}><div style={{fontSize:9,color:C.muted,fontWeight:700,marginBottom:3}}>CAUSES</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{f.causes}</div></div>
    ...
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      ...
      {CT_PATTERNS.map((p,i)=>(
        <div key={i} onClick={()=>setExpandedCT(expandedCT===i?null:i)}
          ...
          {expandedCT===i&&(
            <div style={{borderTop:'1px solid rgba(36,63,82,0.50)',paddingTop:10,display:'flex',flexDirection:'column',gap:8}}>
              <div style={{background:'rgba(255,255,255,0.14)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(36,63,82,0.65)'}}><div style={{fontSize:9,color:C.muted,fontWeight:700,marginBottom:3}}>MEANING</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{p.meaning}</div></div>

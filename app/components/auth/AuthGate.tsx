...
export default function AuthGate({ children, guest = null, allowGuest = true, locale = 'en' }: AuthGateProps) {
  ...
  useEffect(() => {
    ...
    async function restore() {
      ...
      if (error) {
        setState({ status: 'error', user: null, message: 'Unable to restore your session.' })
        return
      }
      ...
      if (profileResult.error) {
        setState({ status: 'error', user: null, message: 'Unable to prepare your profile.' })
        return
      }
  ...
  if (state.status === 'loading') {
    return <GateStatus title="Restoring your session" detail="Preparing Cliniverse securely…" />
  }

  if (state.status === 'error') {
    return <GateStatus title="Account unavailable" detail={state.message} />
  }
...
function GateStatus({ title, detail }: { title: string; detail: string }) {
  return (
    <main style={{ minHeight: '100dvh', background: '#080C16', color: '#F8FAFC', display: 'grid', placeItems: 'center', padding: 24 }}>
      <div role="status" aria-live="polite" style={{ maxWidth: 420, width: '100%', border: '1px solid rgba(148,163,184,0.2)', borderRadius: 20, background: '#111827', padding: 20 }}>
        <div style={{ fontSize: 16, fontWeight: 800 }}>{title}</div>
        <div style={{ marginTop: 7, color: '#94A3B8', fontSize: 13, lineHeight: 1.6 }}>{detail}</div>
      </div>
    </main>
  )
}

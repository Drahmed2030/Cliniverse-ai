...
export default function HealthInsights({ xp=0, casesCompleted=0, mcqCorrect=0, mcqTotal=0, streak=0 }: {
  ...
  return (
    <div style={{ fontFamily:'-apple-system,sans-serif', paddingBottom:20 }}>
      ...
      {/* Tab Toggle */}
      <div style={{ display:'flex', gap:8, marginBottom:16, background:'rgba(255,255,255,0.88)', backdropFilter:'blur(12px)', borderRadius:16, padding:5, border:'1px solid rgba(0,196,180,0.25)' }}>
      ...
      {activeTab === 'insights' && (
        <div>
          ...
          <div style={{ background:'rgba(255,255,255,0.88)', backdropFilter:'blur(20px)', borderRadius:18, padding:18, marginBottom:14, border:'1px solid rgba(0,196,180,0.25)', boxShadow:'none' }}>
              ...
              {['M','T','W','T','F','S','S'].map((day,i)=>{
                ...
                return (
                  <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                    <div style={{ width:'100%', height:`${h}%`, borderRadius:6, background:isToday?'linear-gradient(180deg,#0a84ff,#8b5cf6)':'rgba(0,196,180,0.15)', transition:'height 1s ease', boxShadow:isToday?'0 4px 12px rgba(0,196,180,0.30)':'none' }}/>
                    <div style={{ fontSize:9, color:isToday?'#00C4B4':'#94a3b8', fontWeight:isToday?800:600 }}>{day}</div>
                  </div>
                )
              })}
            </div>
      ...
      {activeTab === 'drugs' && (
        <div>
          <div style={{ background:'rgba(255,255,255,0.88)', backdropFilter:'blur(20px)', borderRadius:20, padding:18, marginBottom:14, border:'1px solid rgba(0,196,180,0.25)', boxShadow:'none' }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.88)', backdropFilter:'blur(20px)', borderRadius:18, padding:16, border:'1px solid rgba(0,196,180,0.25)' }}>
            ...
            {[
              ...
              <div key={item.combo} onClick={()=>{
                ...
                const result = INTERACTIONS[key1]||INTERACTIONS[key2]
                if(result){setInteraction(result);setNoInteraction(false)}
              }} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:12, background:'rgba(255,255,255,0.88)', marginBottom:6, cursor:'pointer', border:'1px solid rgba(0,0,0,0.05)' }}>

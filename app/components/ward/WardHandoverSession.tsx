...
import { draftHandover, type HandoverAction } from '../../lib/ward/handoverSession'
...
function AccountSession({owner}:{owner:string}) {
  const [session, setSession] = useState(() => createPractice('current-status'))
  ...
  function act(action: HandoverAction) {
    ...
    setSession(current => {
      try { return advancePractice(current, action, at) } catch { return current }
    })
  }
  return <section aria-labelledby="ward-handover-title" style={card}>
    ...
    <p style={{ color: 'var(--cv-text-secondary)', marginTop: 0 }}>WARD · HANDOVER PRACTICE</p>
    <h2 id="ward-handover-title">A clearer handover</h2>
    <p><strong>Scenario: {scenario.title}</strong> · Same fictional case, a different documentation task.</p>
    ...
    {session.stage === 'gaps' && <><h3>{scenario.question}</h3><div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}><button type="button" style={button} onClick={() => act(scenario.incorrect)}>{scenario.incorrectLabel}</button><button type="button" style={button} onClick={() => act(scenario.correct)}>{scenario.correctLabel}</button></div><p role="status" aria-live="polite">{session.feedback}</p></>}
    ...
    <p style={{ color: 'var(--cv-text-secondary)', fontSize: '0.875rem' }}>Saved checkpoints resume here on the same content version. Unsaved changes are lost when you leave. Your latest saved practice also appears in Progress, separately from competency scores.</p>
    </fieldset>
  </section>
}

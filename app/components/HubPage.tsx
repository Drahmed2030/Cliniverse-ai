      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:'linear-gradient(180deg,#0d1220,#080c14)', borderRadius:'28px 28px 0 0', border:`1px solid ${section.color}25`, padding:'24px 20px 40px', maxHeight:'80vh', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {(section.cases||[]).map((c:any)=>(
            <div key={c.id} onClick={()=>{ if(!c.free&&!isPro){setShowUpgrade(true);return} onCase(c.id);onClose() }} style={{ background:'rgba(255,255,255,0.12)', borderRadius:18, padding:'14px 16px', border:`1px solid ${c.color||section.color}25`, cursor:'pointer', display:'flex', alignItems:'center', gap:12 }}>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <div style={{ overflowX:'auto', display:'flex', gap:12, paddingBottom:4 }}>
            {[
              ...
              <div key={i} onClick={()=>setShowLive(true)} style={{ minWidth:200, background:'rgba(255,255,255,0.12)', borderRadius:18, padding:'14px', border:`1px solid ${c.color}25`, cursor:'pointer', flexShrink:0 }}>

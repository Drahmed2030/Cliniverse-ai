...
export default function RapidFire({ onXP }:Props) {
  ...
  if(phase==='intro')return(
    <div style={{padding:'0 4px'}}>
      <div style={{background:'linear-gradient(145deg,#F0F6FF,#E8F4FF)',borderRadius:24,padding:28,marginBottom:16,border:'1px solid rgba(255,69,58,0.3)',textAlign:'center',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{fontSize:26,fontWeight:900,color:'#0A1628',marginBottom:6,letterSpacing:-0.5}}>Rapid Fire</div>
        ...
        <div style={{display:'flex',gap:10,justifyContent:'center',marginBottom:24}}>
          {[['30','Questions'],['3 min','Time'],['⚡','XP']].map(([v,l])=>(
            <div key={l} style={{background:'rgba(255,255,255,0.88)',borderRadius:14,padding:'12px 16px',border:'1px solid rgba(255,255,255,0.18)'}}>
              <div style={{fontSize:18,fontWeight:900,color:'#0A1628'}}>{v}</div>
              <div style={{fontSize:10,color:'var(--text-secondary,rgba(10,22,40,0.55))',marginTop:2}}>{l}</div>
            </div>
          ))}
        </div>
        <button onClick={()=>setPhase('game')} style={{background:'linear-gradient(135deg,#ff453a,#ff9f0a)',border:'none',borderRadius:18,padding:'16px 40px',fontSize:17,fontWeight:800,color:'#0A1628',cursor:'pointer',width:'100%',boxShadow:'0 8px 32px rgba(255,69,58,0.4)'}}>Start ⚡</button>
      </div>
    </div>
  )
  ...
  if(phase==='result'){
    ...
    return(
      <div style={{padding:'0 4px'}}>
        <div style={{background:'linear-gradient(145deg,#F0F6FF,#E8F4FF)',borderRadius:24,padding:24,marginBottom:14,border:'1px solid rgba(139,92,246,0.3)',textAlign:'center'}}>
          ...
          <button onClick={()=>navigator.share?navigator.share({text:shareText}):navigator.clipboard.writeText(shareText)} style={{width:'100%',padding:'15px',borderRadius:16,border:'none',background:'var(--bg-base,#F7F9FC)',color:'#0A1628',fontSize:15,fontWeight:700,cursor:'pointer',marginBottom:10}}>📱 Share Score</button>
  ...
  return(
    <div style={{padding:'0 4px',position:'relative'}}>
      {showStreak&&<div style={{position:'fixed',top:'20%',left:'50%',transform:'translateX(-50%)',zIndex:999,background:'linear-gradient(135deg,#ff9f0a,#ff6b35)',borderRadius:20,padding:'12px 24px',fontSize:18,fontWeight:900,color:'#0A1628',boxShadow:'0 8px 32px rgba(255,159,10,0.5)'}}>🔥 {streak} STREAK!</div>}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,background:'rgba(255,255,255,0.92)',borderRadius:16,padding:'10px 14px',border:'1px solid rgba(10,132,255,0.12)'}}>
        ...
        <div style={{textAlign:'center'}}><div style={{fontSize:18,fontWeight:900,color:'#0A1628'}}>{qIndex+1}<span style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))'}}>/30</span></div><div style={{fontSize:9,color:'var(--text-secondary,rgba(10,22,40,0.55))'}}>Q</div></div>
      ...
      <div style={{background:'rgba(255,255,255,0.92)',borderRadius:20,padding:'18px 16px',marginBottom:14,border:'1px solid rgba(36,63,82,0.60)',minHeight:80,display:'flex',alignItems:'center'}}><div style={{fontSize:16,fontWeight:700,color:'#0A1628',lineHeight:1.6}}>{q.q}</div></div>

...
export default function LabModule({ onXP }:{ onXP?:(n:number)=>void }) {
  ...
  if (view === 'menu') return (
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      <div style={{background:'linear-gradient(145deg,#F0F6FF,#E8F4FF)',borderRadius:22,padding:'18px',marginBottom:16,border:'1px solid rgba(0,196,180,0.2)'}}>
          ...
          <div key={i} onClick={()=>setExpandedTest(expandedTest===`${selectedPanel}-${i}`?null:`${selectedPanel}-${i}`)}
            ...
            {expandedTest===`${selectedPanel}-${i}`&&(
              <div style={{borderTop:'1px solid rgba(36,63,82,0.50)',paddingTop:10,display:'flex',flexDirection:'column',gap:8}}>
                <div style={{background:'rgba(10,132,255,0.08)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(0,196,180,0.15)'}}><div style={{fontSize:9,color:'#00C4B4',fontWeight:700,marginBottom:3}}>LOW →</div><div style={{fontSize:12,color:'rgba(10,22,40,0.80)',lineHeight:1.5}}>{t.low}</div></div>
                <div style={{background:'rgba(255,69,58,0.08)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(255,69,58,0.15)'}}><div style={{fontSize:9,color:'#ff453a',fontWeight:700,marginBottom:3}}>HIGH →</div><div style={{fontSize:12,color:'rgba(10,22,40,0.80)',lineHeight:1.5}}>{t.high}</div></div>
              </div>
            )}
          </div>
        ))}
      ...
      return (
        <div style={{fontFamily:'-apple-system,sans-serif',textAlign:'center',padding:'40px 20px'}}>
          ...
          <div style={{display:'flex',gap:10}}>
            ...
            <button onClick={()=>{onXP&&onXP(xp);setView('menu')}} style={{flex:1,padding:'14px',borderRadius:16,border:'none',background:'linear-gradient(135deg,#bf5af2,#8b5cf6)',color:'#0A1628',fontSize:14,fontWeight:800,cursor:'pointer'}}>+{xp} XP ✓</button>
          </div>
        </div>
      )
    }
      ...
      <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
        ...
        {ans!==null&&(
          <div>
            ...
            <button onClick={()=>{setQIdx(i=>i+1);setAns(null)}} style={{width:'100%',padding:'14px',borderRadius:16,border:'none',background:'linear-gradient(135deg,#bf5af2,#8b5cf6)',color:'#0A1628',fontSize:14,fontWeight:800,cursor:'pointer'}}>

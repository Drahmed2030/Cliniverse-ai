...
const COLORS: Record<string,string> = {
  Cardiology:'#ff453a','Critical Care':'#ff9f0a',Respiratory:'#0a84ff',
...
export default function Guidelines() {
  ...
  return (
    <div style={{padding:'0 16px',paddingBottom:100}}>
      ...
      <div style={{display:'flex',gap:6,overflowX:'auto',marginBottom:16,paddingBottom:4,scrollbarWidth:'none'}}>
        {SPECS.map(s=>(
          <button key={s} onClick={()=>setSpec(s)}
            style={{flexShrink:0,padding:'6px 14px',borderRadius:20,border:spec===s?'1px solid rgba(139,92,246,0.5)':'1px solid rgba(255,255,255,0.18)',background:spec===s?'rgba(139,92,246,0.3)':'rgba(255,255,255,0.15)',color:spec===s?'white':'rgba(255,255,255,0.5)',fontSize:12,fontWeight:spec===s?700:400,cursor:'pointer',fontFamily:'inherit'}}>
            {s}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div style={{textAlign:'center',padding:40,color:'rgba(255,255,255,0.3)'}}>Loading guidelines...</div>
      ) : (
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {filtered.map(g=>(
            <div key={g.id} onClick={()=>setExpanded(expanded===g.id?null:g.id)}
              style={{background:'rgba(255,255,255,0.11)',borderRadius:18,padding:16,border:'1px solid '+(COLORS[g.specialty]||'#8b5cf6')+'20',cursor:'pointer',transition:'all 0.2s'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    <span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:(COLORS[g.specialty]||'#8b5cf6')+'15',color:COLORS[g.specialty]||'#8b5cf6',fontWeight:700}}>{g.specialty}</span>
                    <span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(255,255,255,0.15)',color:'rgba(255,255,255,0.4)',fontWeight:600}}>{g.source} {g.year}</span>
                  </div>
                </div>
                <span style={{color:'rgba(255,255,255,0.3)',fontSize:18,transition:'transform 0.2s',transform:expanded===g.id?'rotate(90deg)':'none'}}>›</span>
              </div>
              {expanded===g.id&&(
                <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid rgba(255,255,255,0.15)'}}>
                  <div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.7}}>{g.summary}</div>
                  <div style={{marginTop:10,fontSize:11,color:'rgba(255,255,255,0.3)',fontStyle:'italic'}}>Source: {g.source} {g.year}</div>
                </div>
              )}
            </div>
          ))}
          {filtered.length===0&&(
            <div style={{textAlign:'center',padding:40,color:'rgba(255,255,255,0.3)'}}>No guidelines found</div>
          )}
        </div>
      )}
    </div>

      ...
      <div style={{display:'flex',gap:6,overflowX:'auto',marginBottom:16,paddingBottom:4,scrollbarWidth:'none'}}>
        {SPECS.map(s=>(
          <button key={s} onClick={()=>setSpec(s)}
            style={{flexShrink:0,padding:'6px 14px',borderRadius:20,border:spec===s?'1px solid rgba(139,92,246,0.5)':'1px solid rgba(255,255,255,0.18)',background:spec===s?'rgba(139,92,246,0.3)':'rgba(36,63,82,0.65)',color:spec===s?'white':'rgba(255,255,255,0.5)',fontSize:12,fontWeight:spec===s?700:400,cursor:'pointer',fontFamily:'inherit'}}>
            {s}
          </button>
        ))}
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {filtered.map(g=>(
            <div key={g.id} onClick={()=>setExpanded(expanded===g.id?null:g.id)}
              style={{background:'rgba(36,63,82,0.60)',borderRadius:18,padding:16,border:'1px solid '+(COLORS[g.specialty]||'#00C4B4')+'20',cursor:'pointer',transition:'all 0.2s'}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    ...
                    <span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(36,63,82,0.65)',color:'rgba(255,255,255,0.4)',fontWeight:600}}>{g.source} {g.year}</span>
                  </div>
                </div>

      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", maxWidth: 420, width: "100%", margin: "0 auto" }}>
        ...
        {mode === "landing" ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {appleEnabled ? <AuthButton label={t.apple} bg={T.white} color="#0F172A" onClick={() => handleOAuth("apple")} disabled={loading} icon="" /> : null}
            {googleEnabled ? <AuthButton label={t.google} bg="rgba(255,255,255,0.06)" color={T.white} border={"1px solid " + T.border} onClick={() => handleOAuth("google")} disabled={loading} icon="G" /> : null}
            {oauthEnabled ? <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "8px 0" }}><div style={{ flex: 1, height: 1, background: T.border }} /><div style={{ fontSize: 12, color: T.muted }}>{t.or}</div><div style={{ flex: 1, height: 1, background: T.border }} /></div> : null}

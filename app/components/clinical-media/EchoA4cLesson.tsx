...
interface EchoA4cLessonProps { reducedMotion: boolean }
...
export default function EchoA4cLesson({ reducedMotion }: EchoA4cLessonProps) {
  ...
  async function updatePreviewCompetency() {
    const selected = answers['view-identity']
    const taskId = COMPETENCY_TASK_BY_QUESTION['view-identity']
    if (!selected || !taskId) return
    try {
      const output = await submitEchoPreviewCompetency({
        userId: 'preview-session', taskId, selectedOptionId: selected, confidence,
        responseTimeMs: Math.max(0, Date.now() - startedAtRef.current), attemptedAt: new Date().toISOString(),
      })
      setMastery(output.mastery)
      setCompetencyStatus(output.persistenceState ? 'degraded' : 'ready')
    } catch {
      // Progressive enhancement: competency failure must never break the governed lesson.
      setCompetencyStatus('degraded')
    }
  }

  async function checkAnswers() {
    const viewIdentity = answers['view-identity']; const visibleLandmarks = answers['visible-landmarks']; const safeConclusion = answers['safe-conclusion']

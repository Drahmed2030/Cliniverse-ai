...
export default function EchoA4cLesson({ learner = false, reducedMotion, onCompetencySignal, onAssessment }: EchoA4cLessonProps) {
  ...
  async function checkAnswers(){
    ...
    onAssessment?.(Object.entries(COMPETENCY_TASK_BY_QUESTION).map(([questionId, taskId]) => ({
      taskId, selectedOptionIds: [completeAnswers[questionId as EchoA4cQuestionId]], confidence,
      responseTimeMs: Math.max(0, Date.now() - (startedAtRef.current ?? Date.now())), attemptedAt,
    })))
  ...
  return <section className={styles.echoLesson} aria-labelledby="echo-a4c-lesson-title" dir="ltr">
    <header className={styles.echoLessonHeader}><div><p className={styles.echoEyebrow}>{learner?'REAL ECHO · A4C NORMAL':'REAL ECHO · A4C NORMAL · PREVIEW'}</p><h2 id="echo-a4c-lesson-title">Recognize the A4C view without over-interpreting a short loop</h2><p>Use the governed cine above to identify the complete view signature, observe cyclical motion and preserve the boundary between view recognition and diagnostic measurement.</p></div><span className={styles.realMediaBadge}>English · A4C recognition</span></header>

...
export default function ClinicalMediaPreview() {
  ...
  return <section className={styles.previewShell} aria-labelledby="clinical-media-preview-title" data-testid="clinical-media-preview" dir="ltr">
    <div className={styles.previewHeader}><div><h2 id="clinical-media-preview-title">{copy.title}</h2><p>{copy.body}</p></div><div className={styles.previewControls}>
      {process.env.NODE_ENV==='development'?<button type="button" className={styles.controlButton} aria-pressed={dcmReview} onClick={()=>{setDcmStatus('checking');setLoopPlayback(!dcmReview);setDcmReview(!dcmReview);setProgram('echo-a4c-normal')}}>DCM candidate · local review only</button>:null}
      <div className={styles.controlSet}><span className={styles.controlLabel}>{CONTROL_COPY.program}</span><div aria-label={CONTROL_COPY.program} className={`${styles.controlGroup} ${styles.programControl}`} role="group">{PROGRAM_ORDER.map(option=><button disabled={dcmReview} aria-pressed={program===option} className={`${styles.controlButton} ${program===option?styles.activeControl:''}`} key={option} onClick={()=>{setProgram(option);setLoopPlayback(false);setExpandedCine(false)}} type="button">{PROGRAM_COPY[option].label}</button>)}</div></div>

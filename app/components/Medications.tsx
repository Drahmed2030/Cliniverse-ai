  ...
  return (
    <div style={{padding:'0 16px',paddingBottom:100}}>
      ...
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search medications..."
        style={{width:'100%',padding:'12px 16px',borderRadius:14,border:'1px solid rgba(255,255,255,0.1)',background:'rgba(255,255,255,0.15)',color:'white',fontSize:14,outline:'none',marginBottom:16,boxSizing:'border-box',fontFamily:'inherit'}}/>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {filtered.map(m=>(
            <div key={m.id} onClick={()=>setExpanded(expanded===m.id?null:m.id)}
              style={{background:'rgba(255,255,255,0.11)',borderRadius:18,padding:16,border:'1px solid rgba(48,209,88,0.15)',cursor:'pointer'}}>
              <div style={{display:'flex',alignItems:'center',gap:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:'flex',gap:6}}>
                    ...
                    {m.approval_date&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(255,255,255,0.15)',color:'rgba(255,255,255,0.4)'}}>{m.approval_date}</span>}
                  </div>
                </div>

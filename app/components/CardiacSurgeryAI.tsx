    ...
    <div style={{padding:'0 4px'}}>
      <div style={{background:'linear-gradient(135deg,rgba(255,69,58,0.12),rgba(0,196,180,0.08))',borderRadius:22,padding:20,marginBottom:16,border:'1px solid rgba(255,69,58,0.2)'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:8}}>
          ...
          <div><div style={{fontSize:20,fontWeight:900,color:'var(--text-primary, white)'}}>Cardiac Surgery AI</div><div style={{fontSize:12,color:'rgba(255,255,255,0.4)'}}>CABG · Valve · Aortic · Transplant</div></div>
      ...
      {CASES.map(c=>(
        <div key={c.id} onClick={()=>{setActive(c);setView('case');setStep(0);setDetail(false)}} style={{background:'rgba(255,255,255,0.14)',borderRadius:20,padding:18,marginBottom:12,border:`1px solid ${c.color}22`,cursor:'pointer'}}>
          <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:10}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:15,fontWeight:800,color:'var(--text-primary, white)'}}>{c.title}</div><div style={{fontSize:12,color:'rgba(255,255,255,0.4)',marginTop:2}}>{c.sub}</div></div>
      ...
      {showAIPanel&&(
        <div style={{background:'rgba(15,5,40,0.97)',borderRadius:18,padding:16,marginBottom:12,border:'1px solid rgba(139,92,246,0.3)'}}>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={ask} style={{width:44,height:44,borderRadius:13,border:'none',background:'linear-gradient(135deg,#00C4B4,#0a84ff)',color:'var(--text-primary, white)',fontSize:18,cursor:'pointer',flexShrink:0}}>→</button>
          </div>
        </div>
      )}

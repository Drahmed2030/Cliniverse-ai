  ...
  if (view === 'menu') return (
    <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
      <div style={{background:'linear-gradient(135deg,rgba(48,209,88,0.12),rgba(10,132,255,0.08))',borderRadius:22,padding:'18px',marginBottom:16,border:'1px solid rgba(48,209,88,0.2)',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{fontSize:24,fontWeight:900,color:C.text,letterSpacing:-0.5,marginBottom:4}}>Clinical Pharmacology</div>
      ...
      {filtered.map((int,i)=>(
        <div key={i} onClick={()=>setSelectedInteraction(selectedInteraction===i?null:i)}
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:selectedInteraction===i?10:0}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:800,color:C.text}}>{int.drug1} + {int.drug2}</div>
          ...
          {selectedInteraction===i&&(
            <div style={{borderTop:`1px solid rgba(36,63,82,0.65)`,paddingTop:10}}>
              <div style={{marginBottom:8}}><div style={{fontSize:9,color:C.muted,fontWeight:700,marginBottom:3}}>MECHANISM</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{int.mechanism}</div></div>
      ...
      {DOSING.map((d,i)=>(
        <div key={i} onClick={()=>setSelectedDrug(selectedDrug===i?null:i)}
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:selectedDrug===i?10:0}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:800,color:C.text}}>{d.drug}</div><div style={{fontSize:11,color:C.sub,marginTop:2}}>{d.dose}</div></div>
          ...
          {selectedDrug===i&&(
            <div style={{borderTop:`1px solid rgba(36,63,82,0.65)`,paddingTop:10,display:'flex',flexDirection:'column',gap:8}}>
              <div style={{background:'rgba(255,69,58,0.08)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(255,69,58,0.15)'}}><div style={{fontSize:9,color:'#ff453a',fontWeight:700,marginBottom:3}}>🫘 RENAL ADJUSTMENT</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{d.renal}</div></div>
              <div style={{background:'rgba(10,132,255,0.08)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(0,196,180,0.15)'}}><div style={{fontSize:9,color:'#00C4B4',fontWeight:700,marginBottom:3}}>📊 MONITORING</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{d.monitoring}</div></div>
              <div style={{background:'rgba(255,214,10,0.08)',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(255,214,10,0.15)'}}><div style={{fontSize:9,color:'#ffd60a',fontWeight:700,marginBottom:3}}>⭐ SPECIAL NOTE</div><div style={{fontSize:12,color:'var(--text-secondary,rgba(10,22,40,0.55))',lineHeight:1.5}}>{d.special}</div></div>
            </div>
          )}
        </div>
      ))}
    ...
    if (qIdx >= QUIZ_Q.length) {
      ...
      return (
        <div style={{fontFamily:'-apple-system,sans-serif',textAlign:'center',padding:'40px 20px'}}>
          ...
          <div style={{fontSize:28,fontWeight:900,color:C.text,marginBottom:4}}>{pct}%</div>
          ...
          <div style={{display:'flex',gap:10}}>
            ...
            <button onClick={()=>{onXP&&onXP(xp);setView('menu')}} style={{flex:1,padding:'14px',borderRadius:16,border:'none',background:'var(--bg-base,#F7F9FC)',color:'var(--text-primary, #0A1628)',fontSize:14,fontWeight:800,cursor:'pointer'}}>+{xp} XP ✓</button>
          </div>
        </div>
      )
    }
    ...
    return (
      <div style={{fontFamily:'-apple-system,sans-serif',paddingBottom:20}}>
        ...
        <div style={{height:3,background:'rgba(255,255,255,0.12)',borderRadius:2,overflow:'hidden',marginBottom:14}}>
          <div style={{height:'100%',width:`${(qIdx/QUIZ_Q.length)*100}%`,background:'var(--bg-base,#F7F9FC)',borderRadius:2,transition:'width 0.4s',boxShadow:'0 0 8px rgba(48,209,88,0.5)'}}/>
        </div>
        ...
        {ans!==null&&(
          <div>
            ...
            <button onClick={()=>{setQIdx(i=>i+1);setAns(null)}} style={{width:'100%',padding:'14px',borderRadius:16,border:'none',background:'var(--bg-base,#F7F9FC)',color:'var(--text-primary, #0A1628)',fontSize:14,fontWeight:800,cursor:'pointer',boxShadow:'0 6px 20px rgba(48,209,88,0.35)'}}>

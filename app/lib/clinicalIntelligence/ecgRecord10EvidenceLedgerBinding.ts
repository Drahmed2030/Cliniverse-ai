...
export function evaluateEcgRecord10EvidenceBinding() {
  const integrity = validateEvidenceLedgerIntegrityV2(ECG_RECORD_10_EVIDENCE_EVENTS)
  const hasDeviceBaseline = ECG_RECORD_10_EVIDENCE_EVENTS.some(event =>
    event.kind === 'DEVICE_BASELINE_BOUND' && event.decision === 'PASS',
  )
  const hasPromotion = ECG_RECORD_10_EVIDENCE_EVENTS.some(event =>
    event.kind === 'PROMOTION_DECIDED' && event.decision === 'PROMOTE',
  )

  return {
    integrityValid: integrity.valid,
    blockers: integrity.blockers,
    evidenceLedgerBound: integrity.valid,
    deviceBaselineBound: hasDeviceBaseline,
    learnerReady: false as const,
    promotionAuthorized: hasPromotion,
    decision: integrity.valid && !hasDeviceBaseline && !hasPromotion ? 'HOLD' as const : 'REVIEW' as const,
  }
}

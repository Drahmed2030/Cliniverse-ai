...
import {
  createPathwayRegistrySnapshot,
  type PathwayRegistrySnapshot,
} from './pathwayRegistrySnapshot.ts'

// PathwayReplayAgents — Batch 7, ADAPTED from ops/platform-governance-v1's
// app/lib/cardiology/pathwayReplayAgents.ts. Logic is reused nearly as-is
// (deterministic timeline normalization, KPI computation, gap
// identification — network-free, no AI-generated workflow decisions); the
// only structural change is swapping the branch's much larger unmerged
// "Medical Operations Registry" schema for this repo's additive, scoped
// PathwayRegistrySnapshot (see pathwayRegistrySnapshot.ts for why).
...
export function runPathwayReplay(input: PathwayReplayInput): PathwayReplayReport {
  if (input.dataMode !== 'fictional-simulation') {
    throw new Error('Pathway Replay Intelligence accepts fictional simulation data only.')
  }

  const registry = createPathwayRegistrySnapshot(input.primaryRule.referenceIds)

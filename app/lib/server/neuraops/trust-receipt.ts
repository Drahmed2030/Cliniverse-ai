...
export type NeuraOpsTrustReceipt = {
  ...
  diagnosticReason?: NeuraOpsProbeResult['diagnosticReason']
}
...
export function createNeuraOpsTrustReceipt(input: {
  ...
  return {
    ...
    ...(input.result.diagnosticReason == null ? {} : { diagnosticReason: input.result.diagnosticReason }),
  }
}
...
export async function recordNeuraOpsTrustReceipt(input: {
  ...
  await input.recorder.record({
    ...
    attributes: {
      ...
      diagnostic_reason: input.receipt.diagnosticReason ?? null,
    },
  })
}

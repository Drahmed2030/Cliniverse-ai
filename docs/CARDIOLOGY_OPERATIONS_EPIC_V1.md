...
Release boundary: Fictional local simulation behind verified PRO entitlement

## Executive decision

Cardiology Operations is implemented as one product Epic with isolated modules that share one versioned domain model. The Care release candidate exposes it only after the authenticated subscription authority returns PRO.

The Epic does not replace the existing Ward Simulation. Both workspaces remain available under Care. Ward remains the default free surface and Cardiology Operations is a PRO learning workspace.

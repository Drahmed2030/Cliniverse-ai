...
The gateway also accepts Google's official `GOOGLE_API_KEY` alias and the project's existing server-only `GOOGLE_AI_API_KEY` compatibility alias. `GEMINI_API_KEY` remains the preferred name for new environments.

All populated aliases must contain the same value. If different values are present, readiness reports `keyConfiguration: "conflict"`, the provider call is blocked, and no secret value is revealed. Prefer keeping only `GEMINI_API_KEY` for the strategy Preview after a replacement key is verified; remove obsolete aliases rather than leaving multiple values that can silently shadow one another.

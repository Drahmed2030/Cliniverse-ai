...
The checked-in generated `ios/App/App/PrivacyInfo.xcprivacy` is still non-authoritative because CI recreates `ios/`. The authoritative StoreKit resubmission candidate declares linked name, email, user ID and purchase history plus unlinked technical diagnostic data for app functionality. Specialty/country editing, app analytics and tracking remain outside the submitted surface. Tracking is false and no tracking domains are declared.
